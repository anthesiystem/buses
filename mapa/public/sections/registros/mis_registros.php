...
try {
    ...
    $filtroBus = isset($_GET['bus']) && $_GET['bus'] !== '' ? (int)$_GET['bus'] : null;
    $filtroEngine = isset($_GET['engine']) && $_GET['engine'] !== '' ? (int)$_GET['engine'] : null;
    $filtroTecnologia = isset($_GET['tecnologia']) && $_GET['tecnologia'] !== '' ? (int)$_GET['tecnologia'] : null;
    $filtroEtapa = isset($_GET['etapa']) && $_GET['etapa'] !== '' ? (int)$_GET['etapa'] : null;

    // construir condiciones WHERE - Solo registros activos
    $whereConditions = ['r.activo = b\'1\'']; // Solo mostrar registros activos
    $params = [];
    ...
    if ($filtroBus !== null) {
        $whereConditions[] = "r.Fk_bus = ?";
        $params[] = $filtroBus;
    }

    if ($filtroEngine !== null) {
        $whereConditions[] = "r.Fk_motor_base = ?";
        $params[] = $filtroEngine;
    }

    if ($filtroTecnologia !== null) {
        $whereConditions[] = "r.Fk_tecnologia = ?";
        $params[] = $filtroTecnologia;
    }

    if ($filtroEtapa !== null) {
        $whereConditions[] = "r.Fk_etapa = ?";
        $params[] = $filtroEtapa;
    }
    ...
    SELECT
      ...
      COALESCE(e.descripcion,'—')  AS Entidad,
      COALESCE(b.descripcion,'—')  AS Bus,
      COALESCE(en.descripcion,'—') AS Engine,
      COALESCE(CONCAT(t.numero_version, ' - ', t.descripcion),'—') AS Tecnologia,
      COALESCE(c.descripcion,'—')  AS Categoria,
      COALESCE(eb.descripcion,'—') AS Estado,
      ...
      <tr>
        ...
        <td class='actions text-end'>
          <div class='btn-group'>
            ...
            <button class='btn btn-outline-danger btn-sm' onclick='desactivarRegistro($id)'>
              <i class='bi bi-trash'></i><span class='text ms-1'>Desactivar</span>
            </button>
          </div>
        </td>
      </tr>";
